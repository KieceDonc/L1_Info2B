...
public class TransfertDlg extends javax.swing.JDialog {
    ...
    private void initPanneauGauche(){
        ...
        for(int x=0;x<t;x++){
            BoutonImage currentBI = new BoutonImage();
            currentBI.setPreferredSize(new Dimension(buttonSize,buttonSize));
            PanneauG.add(currentBI);
        }
        this.pack();
    ...
    private void initPanneauDroit(){
        ...
        for(int x=0;x<t;x++){
            BoutonImage currentBI = new BoutonImage();
            currentBI.setPreferredSize(new Dimension(buttonSize,buttonSize));
            currentBI.setName(lcs.getPerso(x).getFamille().getNom());
            currentBI.addActionListener(new ActionListener(){
                @Override
                public void actionPerformed(ActionEvent e) {
                    boutonActionPerformed(e);
                }                
            });
            PanneauD.add(currentBI);
        }
        this.pack();
        dessinePanneau(PanneauD,lstPlayers.getJoueur(indjs).getPaquet());
    ...
    public void dessinePanneau(JPanel jp, LesPersonnages lc){
        ...
        for(int x=0;x<size;x++){
            BoutonImage currentBI = (BoutonImage) jp.getComponent(x);
            currentBI.setImage(lc.getPerso(x).getPhoto());        
        }    
    }
